...
export const Hn = forwardRef(({ depth = 'h1', ...props }: Props, ref: Ref<HTMLParagraphElement>) => {
  ...
  useEffect(() => {
    ...
    const nextHeadingLevel = `h${Number(depth.slice(-1)) <= 6 ? Number(depth.slice(-1)) + 1 : 6}` as keyof typeof Comp;
